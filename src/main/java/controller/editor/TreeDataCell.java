...
public class TreeDataCell extends TreeCell<TreeDataBase> {
	...
	public TreeDataCell(EditorTabController editorTabController) {
	...
	protected void updateItem(TreeDataBase item, boolean empty) {
		...
		if (empty || item == null) {
			...
			editButton.setOnAction(event -> editorTabController.startTreeDataItemEdit(this.getItem()));
			deleteButton.setOnAction(event -> editorTabController.deleteTreeDataItem(this.getItem()));
			...
			if (getTreeView().getTreeItemLevel(getTreeItem())==1) {
				displayTextLabel.setStyle("-fx-font-weight: bold;");
				...
				displayTextLabel.setStyle("-fx-font-weight: normal;");
				rootRegion.setStyle("-fx-border-color: transparent;");
				rootRegion.setMaxWidth(232d);
