...
public class TraceAttributeCell extends TreeCell<TreeDataBase> {
	...
	public TraceAttributeCell(GenerationTabController editorTabController) {
		this.generationTabController = editorTabController;
	}
	...
	protected void updateItem(TreeDataBase item, boolean empty) {
		...
		if (empty || item == null) {
			...
			editButton.setOnAction(event -> generationTabController.startTreeDataItemEdit(this.getItem()));
			deleteButton.setOnAction(event -> generationTabController.deleteTreeDataItem(this.getItem()));
